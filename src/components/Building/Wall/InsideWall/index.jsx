...
const InsideWall=({modelType}) => {
	...
	rightDoorData.forEach(item => {
		...
		if(item.groupType === 'DoubleDoorModel') {
			if(item.doorType === 'double_pattern_door' && doubleDoorClick) {
	...
	return (
		<>
			<group >
				...
				<mesh name='rightwall' rotation={[0,Math.PI/2,0]} position={[-width/2,0,0]}>
				...
				<mesh name='leftwall' rotation={[0,Math.PI/2,0]} position={[width/2-1.5,0,0]}>
